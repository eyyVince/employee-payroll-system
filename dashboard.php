...
<div class="main-panel">
    ...
    <div class="tabs">
        ...
        <div class="tab" data-target="sms">Payroll via SMS</div>
    ...
    <h2>Generate Payroll Receipt via SMS</h2>

    <form id="smsPayrollForm" style="max-width:500px;">

        <label>Select Employee</label>
        <select id="smsEmployee" required>
            <option value="">-- Select Employee --</option>
            <?php
            $smsEmp = $conn->query("SELECT full_name, phone FROM employee_data");
            while ($e = $smsEmp->fetch_assoc()):
            ?>
        ...
        <input type="number" id="smsGross" required>

        <label>Deductions</label>
        <input type="number" id="smsDeduct" required>

        <label>Generated SMS Message</label>
        <textarea id="smsMessage" rows="6" readonly style="width:100%;"></textarea>

        <button type="button" onclick="generateSMS()">Generate SMS</button>
        <button type="button" onclick="sendSMS()" style="margin-top:10px;">
            Send SMS (Demo)
        </button>
    </form>
    ...
    <script>
        function generateSMS() {
            const emp = document.getElementById("smsEmployee");
            const phone = emp.value;
            const name = emp.options[emp.selectedIndex].text;
            const gross = parseFloat(document.getElementById("smsGross").value) || 0;
            const deduct = parseFloat(document.getElementById("smsDeduct").value) || 0;
